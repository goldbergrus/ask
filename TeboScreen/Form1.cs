...
namespace ask
{
    public partial class Form1 : Form
    {
        ...
        public bool searchAsPicture = false;
        Graphics _g;
        Tesseract _t;
        ...
        const string _browserPath = @"C:\Program Files\Waterfox\waterfox.exe";
        const string _URI = @"http://images.google.com/searchbyimage/upload";
            ...
            if (bounds.Width > 0 && bounds.Height > 0)
            {
                using (Bitmap bmp = new Bitmap(bounds.Width, bounds.Height))
                {
                    using (Graphics g = Graphics.FromImage(bmp))
                    {
                        ...
                        _t.SetImage(new Image<Bgr, byte>(bmp));
                        _t.Recognize();
                        var text = _t.GetUTF8Text().Replace("\r\n", " ").Replace("?", "").Replace(".", "").Trim();
                        //bmp.Save(@"C:\Users\Administrator\Nox_share\ImageShare\Screenshots\Dropbox\Screenshots\new.bmp", ImageFormat.Bmp);
                        if (!String.IsNullOrEmpty(text) && !searchAsPicture)
                        {
                            ProcessStartInfo startInfo = new ProcessStartInfo();
                            startInfo.FileName = _browserPath;
                            startInfo.Arguments = @"""https://www.google.ru/search?q=" + text.Replace(@"""", "") + @"""";
                            //startInfo.Arguments = @"""https://yandex.ru/search/?text=" + text + @"""";
                            try
                            {
                        ...
                        /* else
                         {
                             string part1 = @"-----------------------------7dd2db3297c2202Content-Disposition: form-data; name=""encoded_image""; filename=""fileName""Content - Type: image / jpeg";
                             string part2 = @"-----------------------------7dd2db3297c2202Content-Disposition: form-data; name=""image_content""---------------------------- - 7dd2db3297c2202--";
                             byte[] data = Combine(Encoding.UTF8.GetBytes(part1), ImageToByte2(bmp), Encoding.UTF8.GetBytes(part2));
        ...
        private void DrawSelection()
        {
            ...
            _g.DrawRectangle(EraserPen, CurrentTopLeft.X, CurrentTopLeft.Y, CurrentBottomRight.X - CurrentTopLeft.X, CurrentBottomRight.Y - CurrentTopLeft.Y);
            ...
            _g.DrawRectangle(MyPen, CurrentTopLeft.X, CurrentTopLeft.Y, CurrentBottomRight.X - CurrentTopLeft.X, CurrentBottomRight.Y - CurrentTopLeft.Y);
        }
